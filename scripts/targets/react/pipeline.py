...
两者各自独立，互不交叠。
    ...
    5. HtmlToReactStage    — HTML/CSS → JSX + CSS + 复制图片到 ``react/src/assets/images/``
    6. ReactScaffoldStage  — 生成 Vite 脚手架到 ``react/``
"""
...
def build_react_pipeline(ctx: PipelineContext) -> Pipeline:
    return Pipeline([
        # React target 的前 4 段在生成 HTML 中间产物，因此显式落到 html/ 子目录，
