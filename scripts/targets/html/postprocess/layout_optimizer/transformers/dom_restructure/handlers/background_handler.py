"""背景剥离 / 吸收 / 多层合并 - Handler 版本

将原 BackgroundMixin 的功能转换为 Handler 类，维持所有原有逻辑。
通过继承 DOMHandler 基类获得对主对象的访问。
"""
...
class BackgroundHandler(DOMHandler):
    ...
    提供的接口：
    - extract_leaves(): 识别并剥离全覆盖背景层
    - absorb_normal_backgrounds(): 吸收背景层到父容器 CSS
    - 统一闸门: passes_safety_filter() / is_absorbable_bg_leaf()
    """
    ...
    def extract_leaves(
        ...
        基础闸门：所有候选必须先通过 passes_safety_filter
          - data_type == 'image'
          - mix-blend-mode 缺失或 'normal'
          - opacity 缺失或 ≥ 0.99
        与后续 absorb_* 的吸收判定共享同一安全条件，保证"能剥离 ⟺
        ...
        while True:
            ...
            for leaf in remaining:
                ...
                if not self.passes_safety_filter(leaf, styles):
                    continue
    ...
    def absorb_normal_backgrounds(self, group, tree: LayoutNode) -> List[LeafInfo]:
        """识别 stack tree 中"可吸收"的背景 leaf，并将其合并为 group 的 CSS background。"""
        candidates: List[Tuple[LeafInfo, Dict[str, str]]] = []
        
        container_bbox = self.owner._container_css_bbox(group, fallback=tree.bbox)
        ...
        for child in tree.children:
            ...
            if styles is None:
                continue
            if not self.is_absorbable_bg_leaf(leaf, styles):
                continue
