...
在聚类前剥离"高瘦+纵向跨多行+跨过行在X上对齐"的装饰元素。
"""
...
class TallDecorHandler(DOMHandler):
    """高瘦跨行装饰 Handler"""

    def extract_tall_decor_leaves(
        self,
        ...
        """
        if not self.owner.config.enable_tall_decor_extraction:
            return [], leaves
        if len(leaves) < self.owner.config.tall_decor_min_crossed_rows + 1:
            return [], leaves
    ...
    def _pick_one_tall_decor(
        ...
        for cand in ordered:
            ...
            if len(others) < self.owner.config.tall_decor_min_crossed_rows:
                continue
            ...
            if mid <= 0:
                continue
            if ch < mid * self.owner.config.tall_decor_height_ratio:
                continue

            # 条件 2：纵横比 >= aspect_min
            if ch / cw < self.owner.config.tall_decor_aspect_min:
                continue
            ...
            for o in others:
                ...
                if ov / o.bbox.height >= 0.5:
                    crossed.append(o)
            if len(crossed) < self.owner.config.tall_decor_min_crossed_rows:
                continue

            # 条件 4：被跨过的 leaves 之间 X 轴对齐
            if not self._are_x_aligned(
                crossed, self.owner.config.tall_decor_x_align_tolerance):
                continue
