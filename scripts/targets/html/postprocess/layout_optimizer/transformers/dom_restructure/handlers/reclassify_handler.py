...
class ReclassifyHandler(DOMHandler):
    """重分类 Handler - Stack → Col 反向升级"""

    def absorb_container_backgrounds_pass(self):
        ...
        if self.owner.config.enable_stack_to_col_reclassify and absorbed_containers:
            self._reclassify_stacks_after_bg_absorption(absorbed_containers)

        for el in self.owner.soup.find_all(attrs={'data-no-bg-absorb': True}):
    ...
    def _collect_bg_absorb_target_containers(self) -> List:
        ...
        result = []
        result.extend(
            self.owner.soup.find_all(
                'div',
                class_=lambda x: x and 'layer-group' in str(x),
            )
        )
        result.extend(
            self.owner.soup.find_all('div', attrs={'data-virtual': True})
        )
        seen = set()
        unique = []
        for elem in result:
    ...
    def _try_absorb_container_bg(self, container) -> List[LeafInfo]:
        ...
        container_styles = self.owner.css_rules.get(container_css_class)
        if not container_styles:
            return []
        try:
            cw = self.owner.parser.parse_px(container_styles.get('width', '0'))
            ch = self.owner.parser.parse_px(container_styles.get('height', '0'))
        ...
        cover_ratio = self.owner.config.container_bg_cover_ratio
        overflow_tol = self.owner.config.container_bg_overflow_tolerance_px
        ...
        absorbed = self.owner.background._merge_bg_candidates_into_container_css(
    ...
    def _collect_direct_children_info(self, container) -> List[Dict]:
        ...
        for child in list(container.find_all(recursive=False)):
            ...
            styles = self.owner.css_rules.get(css_class)
            if styles is None:
                continue
            ...
            try:
                left = self.owner.parser.parse_px(styles.get('left', '0'))
                top = self.owner.parser.parse_px(styles.get('top', '0'))
                width = self.owner.parser.parse_px(styles.get('width', '0'))
                height = self.owner.parser.parse_px(styles.get('height', '0'))
            except (ValueError, AttributeError):
                continue
    ...
    def _is_absorbable_bg_leaf(self, leaf: LeafInfo, styles: Dict) -> bool:
        """判定 leaf 是否可吸收（由 BackgroundHandler 提供的签名）"""
        data_type = leaf.data_type or ''
        if data_type not in ('image',):
            if 'layer-group' not in (styles.get('class') or []):
                return False
        if leaf.element.get('data-disable-bg-absorb'):
            return False
        return True
    ...
    def _try_reclassify_stack_to_col(self, container) -> bool:
        ...
        if len(children_data) < self.owner.config.reclassify_min_rows:
            return False
        ...
        rows = self.owner.clustering.split_by_rows(leaves_for_split)
        if len(rows) != len(leaves_for_split):
            return False
        if len(rows) < self.owner.config.reclassify_min_rows:
            return False

        # 横向覆盖率校验
        if len(rows) == 2:
            x_ratio_threshold = self.owner.config.reclassify_n2_min_x_overlap
        else:
            x_ratio_threshold = self.owner.config.reclassify_x_overlap_ratio
        ...
        if len(rows) == 2 and gaps:
            if gaps[0] > self.owner.config.reclassify_n2_max_gap_px:
                return False
        elif gaps:
            ...
            if abs(mean_gap) > 0.5:
                ...
                cv = std_gap / abs(mean_gap) if abs(mean_gap) > 0 else 0
                if cv > self.owner.config.reclassify_gap_cv_max:
                    return False
    ...
    def _upgrade_stack_container_to_col(
        ...
        for leaf in sorted_leaves:
            ...
            self.owner.rendering.apply_flex_child_margins(
        ...
        self.owner.stats['dom_restructured'] = self.owner.stats.get('dom_restructured', 0) + 1
