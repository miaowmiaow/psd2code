...
class ClusteringHandler(DOMHandler):
    """空间聚类 Handler"""

    def cluster(self, leaves: List[LeafInfo]) -> LayoutNode:
        """递归聚类：先按行切，多行则包 col；单行走列聚类"""
        if len(leaves) == 1:
            leaf = leaves[0]
            return LayoutNode(kind='leaf', bbox=leaf.bbox, leaf=leaf)

        rows = self.split_by_rows(leaves)

        if len(rows) > 1:
            if self._is_fake_multirow_stack(rows):
                return LayoutNode(
                    kind='stack',
                    bbox=self.owner._envelope([l.bbox for l in leaves]),
                    children=[self._leaf_to_node(l) for l in leaves],
                )

            children_nodes = [self.cluster_row(r) for r in rows]
            return LayoutNode(
                kind='col',
                bbox=self.owner._envelope([l.bbox for l in leaves]),
        ...
        return self.cluster_row(rows[0])

    def _is_fake_multirow_stack(self, rows: List[List[LeafInfo]]) -> bool:
        """判定多行结果是否实为"上下贴边的堆叠装饰"，应回退 stack。"""
        if len(rows) < 2:
            return False
        if len(rows) >= self.owner.config.fake_multirow_max_rows:
            return False
        if any(len(r) > 1 for r in rows):
            return False

        envelopes = [self.owner._envelope([l.bbox for l in r]) for r in rows]
        ratio_threshold = self.owner.config.multi_row_stack_fallback_x_ratio
    ...
    def cluster_row(self, row_leaves: List[LeafInfo]) -> LayoutNode:
        """聚类单行内元素"""
        if len(row_leaves) == 1:
            leaf = row_leaves[0]
            return LayoutNode(kind='leaf', bbox=leaf.bbox, leaf=leaf)

        if self.is_stack_group([l.bbox for l in row_leaves]):
            return LayoutNode(
                kind='stack',
                bbox=self.owner._envelope([l.bbox for l in row_leaves]),
                children=[self._leaf_to_node(l) for l in row_leaves],
            )

        cols = self.split_by_cols(row_leaves)
        if len(cols) > 1:
            children_nodes = [self.cluster(c) for c in cols]
            return LayoutNode(
                kind='row',
                bbox=self.owner._envelope([l.bbox for l in row_leaves]),
                children=children_nodes,
    ...
    def split_by_rows(self, leaves: List[LeafInfo]) -> List[List[LeafInfo]]:
        ...
        ratio_threshold = self.owner.config.row_dominant_overlap_ratio
    ...
    def split_by_cols(self, leaves: List[LeafInfo]) -> List[List[LeafInfo]]:
        """按 X 轴区间聚类（带微重叠容忍）"""
        ...
        for l in ordered[1:]:
            ...
            tolerance = max(min_width * self.owner.config.overlap_split_ratio,
                          self.owner.config.col_gap_px * 0.5)
    ...
    def is_stack_group(self, bboxes: List[BBox]) -> bool:
        """判定是否为叠图组（stack）
        ...
        for a, b in combinations(effective, 2):
            if a.overlap_ratio(b) >= self.owner.config.stack_pair_threshold:
                stack_pairs += 1
        return stack_pairs / total_pairs >= self.owner.config.stack_majority
